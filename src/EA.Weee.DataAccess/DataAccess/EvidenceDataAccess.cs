            ...
            var notes = await context.Notes
               .Where(p =>
                   (!filter.OrganisationId.HasValue || p.Organisation.Id == filter.OrganisationId.Value)
                    ...
                    && (!filter.StartDateSubmitted.HasValue || p.NoteStatusHistory.Any(nsh => nsh.ToStatus.Value == NoteStatus.Submitted.Value && nsh.ChangedDate != null)
                            && p.NoteStatusHistory.Where(nsh => nsh.ToStatus.Value == NoteStatus.Submitted.Value).OrderByDescending(nsh1 => nsh1.ChangedDate).FirstOrDefault().ChangedDate >= filter.StartDateSubmitted)
                    && (!filter.StartDateSubmitted.HasValue || p.NoteStatusHistory.Any(nsh => nsh.ToStatus.Value == NoteStatus.Submitted.Value && nsh.ChangedDate != null)
                            && p.NoteStatusHistory.Where(nsh => nsh.ToStatus.Value == NoteStatus.Submitted.Value).OrderByDescending(nsh1 => nsh1.ChangedDate).FirstOrDefault().ChangedDate <= filter.EndDateSubmitted)

GO
PRINT N'Upserting [Lookup].[ChargeBandAmount]…';
...
-- Upsert using MERGE on Id
MERGE [Lookup].[ChargeBandAmount] AS tgt
USING @src AS src
  ON tgt.[Id] = src.[Id]
WHEN MATCHED THEN UPDATE SET
    tgt.[Amount]               = src.[Amount],
    tgt.[ChargeBand]           = src.[ChargeBand],
    tgt.[CompetentAuthority]   = src.[CompetentAuthority],
    tgt.[VatRegistered]        = src.[VatRegistered],
    tgt.[AnnualTurnoverBand]   = src.[AnnualTurnoverBand],
    tgt.[EEEPlacedOnMarketBand]= src.[EEEPlacedOnMarketBand],
    tgt.[ComplianceYear]       = src.[ComplianceYear],
    tgt.[EffectiveFrom]        = src.[EffectiveFrom]
WHEN NOT MATCHED BY TARGET THEN
    INSERT ([Id],[Amount],[ChargeBand],[CompetentAuthority],[VatRegistered],
            [AnnualTurnoverBand],[EEEPlacedOnMarketBand],[ComplianceYear],[EffectiveFrom])
    VALUES (src.[Id],src.[Amount],src.[ChargeBand],src.[CompetentAuthority],src.[VatRegistered],
            src.[AnnualTurnoverBand],src.[EEEPlacedOnMarketBand],src.[ComplianceYear],src.[EffectiveFrom])
OUTPUT $action, inserted.Id;

PRINT N'Upsert complete.';
GO

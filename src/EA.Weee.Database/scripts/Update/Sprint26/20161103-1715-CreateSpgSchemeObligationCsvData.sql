...
SELECT 
	...
	PrevEeeByCat.ObligationType AS 'ObligationTypeForPreviousYear',
...
FROM [Producer].[RegisteredProducer] RP
	...
	LEFT OUTER JOIN (
		...
		(
			...
			GROUP BY PrevEOA.RegisteredProducerId, PrevEOA.WeeeCategory, PrevRP.ProducerRegistrationNumber, PrevPS.ObligationType
		) AS PrevEee
		PIVOT
		(

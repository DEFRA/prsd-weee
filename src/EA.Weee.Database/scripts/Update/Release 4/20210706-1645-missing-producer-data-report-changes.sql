IF EXISTS (SELECT 1 FROM sys. views WHERE OBJECT_ID=OBJECT_ID('[dbo].[vwMissingProducerDataReturn]'))
BEGIN	
	DROP VIEW [dbo].[vwMissingProducerDataReturn];
END
...
IF EXISTS (SELECT 1 FROM sys. views WHERE OBJECT_ID=OBJECT_ID('[dbo].[vwMissingProducerSubmissionData]'))
BEGIN	
	DROP VIEW [dbo].[vwMissingProducerSubmissionData];
END
...
FROM [Producer].[RegisteredProducer] RP
	...
	LEFT OUTER JOIN (SELECT * FROM vwMissingProducerDataReturn WHERE ComplianceYear = @ComplianceYear AND ObligationType = @ObligationType AND [Quarter] = COALESCE(@Quarter, [Quarter])) AS QuarterType ON QuarterType.RegProdId = RP.Id

...
-- this captures the case where a re-subission has been created but no data has been submitted
...
(
	SELECT
		a.*,
		ROW_NUMBER() OVER (PARTITION BY a.AatfId ORDER BY a.CreatedDate ASC) AS RowNumber
		FROM
			(SELECT DISTINCT 
				a.AatfId,			
				a.Name,
			...
			FROM
				...
				LEFT JOIN  [Identity].[AspNetUsers] u ON u.id = r.SubmittedById) a
...
SELECT 
	...
	CASE WHEN y.RowNumber = 1 THEN 'First submission' ELSE 'Resubmission' END AS ReSubmission
FROM
	@AATFReturn Y
WHERE	

-- Description:	This stored procedure is used to provide the data for the admin report of sent on for treatment
--				that have submitted a return within
...
INNER JOIN [AATF].[ReturnReportOn] ro on ro.ReturnId = r.Id
AND ro.ReportOnQuestionId = 2
...
SELECT u.AatfId, u.ReturnId, u.[Quarter], u.CategoryId, u.Tonnage, SUM(u.value) AS VALUE, OperatorAddressId, SiteAddressId
FROM (
	SELECT r.AatfId, r.ReturnId, r.[Quarter], wsoa.CategoryId,
	  ...
	  ,wso.OperatorAddressId, wso.SiteAddressId
...
SET SiteOperatorData = x.SiteOperator
...
UPDATE #AatfSentOnData
SET SiteOperatorId = s.SiteOperatorId
FROM #AatfSentOnData a
...
BEGIN
	INSERT INTO #AatfSentOnData (AatfId, ReturnId, [Quarter], CategoryId, TonnageType)
	SELECT s.AatfId, s.ReturnId, s.[Quarter], c.Id, 'HouseholdTonnage'
	FROM [Lookup].WeeeCategory c
	LEFT JOIN @SUBMITTEDRETURN s ON 1 = 1
	EXCEPT
	SELECT DISTINCT AatfId, ReturnId, [Quarter], CategoryId, TonnageType FROM #AatfSentOnData
...
,a.Obligation
,a.TotalSent AS 'Total sent on (t)'
, x.*
FROM
	(
	SELECT o.AatfId, r.CompetentAuthorityAbbr, r.PanArea, r.[Quarter], r.SubmittedBy, r.SubmittedDate, 
	r.LocalArea, r.OrganisationName, r.Name, r.ApprovalNumber, o.CategoryId, o.returnId, o.TonnageType, 
	CASE 
		WHEN o.TonnageType = 'HouseholdTonnage'
			THEN 'B2C'
		ELSE 'B2B'
		END AS Obligation, SUM(o.TotalSent) AS TotalSent
FROM #AatfSentOnData o
INNER JOIN @SUBMITTEDRETURN r ON r.AatfId = o.AatfId
	AND R.[Quarter] = o.[Quarter]
	AND R.ReturnId = o.ReturnId
GROUP BY o.AatfId, r.CompetentAuthorityAbbr, r.PanArea, r.[Quarter], r.SubmittedBy, r.SubmittedDate, r.LocalArea, 
...
BEGIN
	SELECT a.CompetentAuthorityAbbr AS 'Appropriate authority', a.PanArea AS 'WROS pan area team', a.LocalArea AS 'EA Area', 
	@ComplianceYear AS 'Year', CONCAT ('Q', a.[Quarter]) AS 'Quarter', a.SubmittedBy AS 'Submitted by', 
	a.SubmittedDate AS 'Date submitted (GMT)', a.OrganisationName AS 'Organisation name', a.Name AS 'Name of AATF', 
	a.ApprovalNumber AS 'Approval number', CONCAT (c.Id, '.', c.Name) AS Category, a.Obligation, a.TotalSent AS 'Total sent on (t)'
	FROM (
		SELECT DISTINCT o.AatfId, r.CompetentAuthorityAbbr, r.PanArea, r.[Quarter], r.SubmittedBy, r.SubmittedDate, r.LocalArea, 
		r.OrganisationName, r.Name, r.ApprovalNumber, o.CategoryId, o.returnId, o.TonnageType, 
		CASE 
				WHEN o.TonnageType = 'HouseholdTonnage'
					THEN 'B2C'

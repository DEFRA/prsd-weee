...
INNER JOIN [AATF].[ReturnReportOn] ro on ro.ReturnId = X.ReturnId
...
BEGIN
	...
	SELECT s.AatfId, s.ReturnId, s.[Quarter], c.Id, 'HouseholdTonnage', 0
	...
	SELECT s.AatfId, s.ReturnId, s.[Quarter], c.Id, 'NonHouseholdTonnage', 0
	FROM [Lookup].WeeeCategory c
	LEFT JOIN @SUBMITTEDRETURN s ON 1 = 1
	EXCEPT
	SELECT DISTINCT AatfId, ReturnId, [Quarter], CategoryId, TonnageType, SiteOperatorId FROM #AatfSentOnData

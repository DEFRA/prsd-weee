...
-- =============================================
ALTER PROCEDURE [AATF].[getAllAatfReuseSitesCsvData]
	@ComplianceYear INT,
	@CA UNIQUEIDENTIFIER,
...
FROM (
	SELECT ra.AatfId, ra.ReturnId, r.ComplianceYear, r.[Quarter], r.CreatedDate, r.SubmittedDate, CONCAT (u.FirstName, ' ', u.Surname) AS SubmittedBy, a.Name, a.ApprovalNumber, CASE 
			WHEN o.Name IS NULL
				THEN o.TradingName
			ELSE o.Name
			END AS OrgName, ca.Abbreviation, pa.Name AS PanName, la.Name AS LaName, ROW_NUMBER() OVER (
			PARTITION BY ra.AatfId, r.[Quarter] ORDER BY r.[Quarter], r.SubmittedDate DESC
			) AS RowNumber
	FROM [AATF].[ReturnAatf] ra
	INNER JOIN [AATF].[Return] r ON r.Id = ra.ReturnId
	INNER JOIN AATF.AATF a ON a.Id = ra.AatfId
		AND a.FacilityType = r.FacilityType
...
AND ro.ReportOnQuestionId = 3 --reuse option

...
SELECT  EEORVA.EeeOutputReturnVersionId as 'EeeReturnVersionId',	
		DENSE_RANK() over
					(
...
SELECT EeeData.EeeReturnVersionId,
			...
			select eee.EeeReturnVersionId, eee.PRN, eee.ApprovalNumber, eee.ComplianceYear, eee.SchemeName, 
				...
				INNER JOIN (
							...
							ON cd.EeeReturnVersionId = eee.EeeReturnVersionId AND cd.SubmittedDate=eee.SubmittedDate
			) AS p
            PIVOT
            (
                    max(p.Tonnage) 
...
SELECT DISTINCT Producers.PRN,  
        ...
        FROM (
                    SELECT PE.EeeReturnVersionId,  						
                    Pe.PRN, PE.ApprovalNumber, PE.SchemeName, PE.ComplianceYear, PE.SubmittedDate, PE.Quarter
			        FROM  #ProducerEEEData PE		
            		group by PE.EeeReturnVersionId,PE.PRN, PE.ApprovalNumber, PE.SchemeName, PE.ComplianceYear, PE.SubmittedDate, PE.Quarter
             ) AS Producers 


LEFT JOIN (
SELECT DISTINCT EEEData.EeeReturnVersionId, EEEData.PRN, EEEData.ApprovalNumber, EEEData.ComplianceYear, EEEData.SubmittedDate, EEEData.Quarter, 
...
) as B2C on (Producers.EeeReturnVersionId = B2C.EeeReturnVersionId and 
...
--B2B 
LEFT JOIN (
SELECT DISTINCT EEEData.EeeReturnVersionId, EEEData.PRN, EEEData.ApprovalNumber, EEEData.ComplianceYear, EEEData.Quarter,EEEDAta.SubmittedDate,
...
) as B2B on (Producers.EeeReturnVersionId = B2B.EeeReturnVersionId and 

...
FROM
	...
	LEFT JOIN [PCS].ObligationScheme os WITH (NOLOCK) ON os.Id = osa.ObligationSchemeId
	LEFT JOIN [PCS].Scheme sc WITH (NOLOCK) ON sc.Id = os.SchemeId AND sc.Id = @SchemeId
WHERE
...
FROM
	#EvidenceSummary s
	INNER JOIN Evidence.vwHouseholdEvidenceSumByCategoryAndRecipient evc WITH (NOLOCK) ON evc.CategoryId = s.CategoryId AND evc.ComplianceYear = @ComplianceYear
	LEFT JOIN [PCS].Scheme sc WITH (NOLOCK) ON sc.OrganisationId = evc.ReceiverOrganisation AND sc.Id = @SchemeId
WHERE
...
FROM
	...
	LEFT JOIN [PCS].Scheme sc WITH (NOLOCK) ON sc.OrganisationId = evc.TransferOrganisation AND sc.Id = @SchemeId
WHERE
	(sc.Id = @SchemeId OR @SchemeId IS NULL)
...
SELECT 
	...
	EvidenceReceivedInTotal AS EvidenceOriginal,
	CASE WHEN EvidenceReceivedInTotal IS NULL AND TransferEvidenceReceivedOut IS NULL THEN NULL ELSE
		COALESCE(EvidenceReceivedInTotal, 0) - COALESCE(TransferEvidenceReceivedOut, 0) END AS EvidenceDifference

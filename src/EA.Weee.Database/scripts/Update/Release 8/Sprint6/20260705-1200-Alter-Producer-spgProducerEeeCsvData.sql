...
-- Modified Date: 07/05/2026 11:00:00 Ian Alvarado - FIX: Use per-compliance-year DirectProducerSubmissionHistory for ProducerName and ProducerCountry
--                                                        rather than the shared root Organisation and DirectRegistrant entity fields,
--                                                        which are no longer synced on submission completion.
...
BEGIN
    ...
    SELECT
        ...
        -- ProducerName: prefer overseas producer name (auth rep), then per-year company name
        --               from submission history, finally fall back to root organisation name
        COALESCE(AP.OverseasProducerName, DPSH.CompanyName, O.[Name], '') AS 'ProducerName',

        -- ProducerCountry: prefer overseas contact country (auth rep), then business address
        --                  country from submission history, finally fall back to root org country
        COALESCE(AC.Name, HIST_OC.Name, OC.Name, '') AS 'ProducerCountry',
    ...
    FROM [Producer].[DirectProducerSubmission] DPS
        ...
        -- Root org address kept as a fall-back for legacy/migrated records without history address
        ...
        -- FIX: Prefer authorised representative recorded in submission history;
        --      fall back to root DirectRegistrant authorised representative for migrated records.
        LEFT JOIN [Producer].[AuthorisedRepresentative] AP
            ON AP.Id = COALESCE(DPSH.AuthorisedRepresentativeId, DR.AuthorisedRepresentativeId)
        ...
        -- FIX: Business address from submission history for per-year country fall-back.
        --      Used in ProducerCountry when no overseas (auth rep) country is present.
        LEFT JOIN [Organisation].[Address] HIST_BA ON HIST_BA.Id  = DPSH.BusinessAddressId
        LEFT JOIN [Lookup].[Country]       HIST_OC ON HIST_OC.Id  = HIST_BA.CountryId

    WHERE RP.Removed = 0
      AND EOA.ObligationType = @ObligationType
      AND DPS.ComplianceYear = (@ComplianceYear + 1)
      AND DPS.PaymentFinished = 1
      AND @SchemeId IS NULL
      AND (@FilterByDirectRegistrant = 1 OR (@FilterBySchemes = 0 AND @FilterByDirectRegistrant = 0))

    -- Pivot EEE data by category

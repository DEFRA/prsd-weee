...
-- =============================================================================================================
-- Modified Date: 23/07/2026 10:30:00 
-- Description: Fixed TradingName and CompanyName columns for Direct Registrants to use year-specific submission history
--              instead of root organisation data, ensuring the selected compliance year data is returned.
-- =============================================================================================================
-- Modified Date: 12/08/2026 12:00:00
...
BEGIN
	...
	-- Only include producers with "Registered" or "Returned" status
	SELECT
		CASE
		...
		-- Producer Information
		-- Fixed: 23/07/2026 Use year-specific submission history TradingName
	...
	WHERE
		...
		(@FilterByDirectRegistrant = 1 OR (@FilterBySchemes = 0 AND @FilterByDirectRegistrant = 0)) AND
		-- Fixed 12082026: Only include Direct Registrants with "Registered" or "Returned" status
		(
			(dps.[Status] = 2 AND dps.PaymentFinished = 1 AND (dps.FinalPaymentSessionId IS NOT NULL OR dps.ManualPaymentReceivedDate IS NOT NULL))
			OR
			(dps.[Status] = 3 AND dps.PaymentFinished = 1 AND (dps.FinalPaymentSessionId IS NOT NULL OR dps.ManualPaymentReceivedDate IS NOT NULL))
		)

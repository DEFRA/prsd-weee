/****** Object:  StoredProcedure [Producer].[spgProducerPublicRegisterCSVDataByComplianceYear]    Script Date: 15/05/2026 15:26:20 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
-- =============================================================================================================
-- Modified Date: 12/08/2026 11:00:00
-- Description: Added filter to return only Direct Registrants with "Registered" or "Returned" status
-- =============================================================================================================
...
UNION ALL
	...
	-- FIX (20260727): Use per-compliance-year DirectProducerSubmissionHistory exclusively
	--                 Remove fallback to root Organisation/DirectRegistrant for year-specific accuracy
	SELECT
		-- CompanyName/ProducerName: use submission history only (no fallback)
		DPSH.CompanyName AS 'CompanyName',
		DPSH.CompanyName AS 'ProducerName',

		-- TradingName: use submission history only (no fallback)
		DPSH.TradingName AS 'TradingName',

		dbo.GetObligationType(EORV.Id) AS 'ObligationType',

		-- Business address: use submission history only (no fallback)
		HIST_BA.Address1 AS 'ROAPrimaryName',
		NULL AS 'ROASecondaryName',
		HIST_BA.Address2 AS 'ROAStreet',
		HIST_BA.TownOrCity AS 'ROATown',
		HIST_BA.CountyOrRegion AS 'ROALocality',
		NULL AS 'ROAAdministrativeArea',
		HIST_BA.PostCode AS 'ROAPostCode',
		HIST_LOC.[Name] AS 'ROACountry',

		-- Telephone/email/fax from business address: use submission history only (no fallback)
		CASE WHEN O.OrganisationType = 1 THEN HIST_BA.Telephone ELSE NULL END AS 'ROATelephone',
		CASE WHEN O.OrganisationType = 1 THEN HIST_BA.Email ELSE NULL END AS 'ROAEmail',
		CASE WHEN O.OrganisationType = 1 THEN HIST_BA.Fax ELSE NULL END AS 'ROAFax',
		...
		-- Authorised representative: use submission history only (no fallback)
	...
	FROM
		...
		-- FIX (20260727): Added compliance year filter to subquery to ensure correct history record
		INNER JOIN (
			SELECT
		...
		-- Business address from submission history (no fallback to root org address)
		...
		-- Authorised representative from submission history (no fallback to DirectRegistrant)

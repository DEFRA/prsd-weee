/****** Object:  StoredProcedure [Evidence].[getAatfEvidenceSummaryTotals]    Script Date: 23/01/2025 14:04:40 ******/
SET ANSI_NULLS ON
GO
...
WHERE
	...
	c.CategoryId >= 1 AND c.CategoryId <= 15
GROUP BY
	c.CategoryId
)
UPDATE
...
FROM
	...
	INNER JOIN
		...
		FROM
			#AatfSummaryTotals s
		WHERE
			s.CategoryId >= 1 AND s.CategoryId <= 15) cs ON s.CategoryId = 1001

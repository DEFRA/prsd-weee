...
#Date format
$newScriptName = (Get-Date -Format yyyyMMdd-HHmmss-) + "$databaseName" + ".sql.temp"

write-host "Building original database..."
& $AliaSQL Create $sqlServerName "$databaseName_Original" "$databaseScripts\Scripts"   
write-host "`n`nGenerating the diff script"
#generate the needed .dacpac (we'll delete it later)
& $SqlPackage /a:Extract /ssn:$sqlServerName /sdn:$databaseName /tf:$databaseScripts$databaseName.dacpac 
#generate the diff script
& $SqlPackage /a:Script /op:$databaseScriptsUpdate\$newScriptName /p:DropObjectsNotInSource=true /p:BlockOnPossibleDataLoss=false /sf:$databaseScripts$databaseName.dacpac /tsn:$sqlServerName /tdn:"$databaseName_Original"
...
& sqlcmd -S $sqlServerName  -Q "DROP DATABASE [$databaseName_Original]"  

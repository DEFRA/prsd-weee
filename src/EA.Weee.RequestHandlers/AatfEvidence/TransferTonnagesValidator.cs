namespace EA.Weee.RequestHandlers.AatfEvidence
{
    ...
    public class TransferTonnagesValidator : ITransferTonnagesValidator
    {
        ...
        {
            var existingNoteTonnage = await evidenceDataAccess.GetTonnageByIds(transferValues.Select(t => t.Id).ToList());
            ...
            foreach (var noteTonnage in existingNoteTonnage)
            {
                ...
                var totalReceivedAvailable = noteTonnage.Received.Value - (noteTonnage.NoteTransferTonnage != null && noteTonnage.NoteTransferTonnage.Any() ?
                                                 noteTonnage.NoteTransferTonnage.Where(nt => nt.Received.HasValue &&
                                                     ...
                                                     !nt.TransferNote.Status.Value.Equals(NoteStatus.Cancelled.Value) &&
                                                     (!existingTransferNoteId.HasValue || nt.TransferNoteId != existingTransferNoteId.Value)).Sum(nt => nt.Received.Value) : 0);

                var requestedTonnage = transferValues.First(t => t.Id == noteTonnage.Id);

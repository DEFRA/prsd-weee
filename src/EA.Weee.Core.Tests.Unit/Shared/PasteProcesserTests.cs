namespace EA.Weee.Core.Tests.Unit.Shared
{
    ...
    public class PasteProcesserTests
    {
        ...
        [Fact]
        public void GivenString_StringShouldBeStrippedOfNonNumericCharactors()
        {
            // test this
            var lstAllowedCharacters = new List<string> { "1", "2", "3", "4", "5", "6", "7", "8", "9", "0" };

            var t = @"1\t2\r\n3\t4\r\n";

            IEnumerable<Char> allowed = t.Select(c => lstAllowedCharacters.Contains(c.ToString()) ? c : ' ');
            string result = new string(allowed.ToArray()).Replace(" ", string.Empty);
        }

        // test with spurious newlines at the end
        ...
        // start with single char

        // char with space after 

        // char with space then space

        // char with space then space and then newline without any more

        // strip out non numeric charactors

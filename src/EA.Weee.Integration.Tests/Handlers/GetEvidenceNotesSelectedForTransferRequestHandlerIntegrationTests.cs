namespace EA.Weee.Integration.Tests.Handlers
{
    ...
    {
        [Component]
        public class WhenIGetNotesSelectedToTransferForAnOrganisationWithExistingTransferNoteParameter : GetEvidenceNotesSelectedForTransferRequestHandlerIntegrationTestBase
        {
            ...
            private readonly Establish context = () =>
            {
                ...
                // this transfer shouldnt affect the available amount as the note is approved but request takes the note id
                var transferTonnages5 = new List<NoteTransferTonnage>()
                {
                    new NoteTransferTonnage(
                        evidenceNoteWithTransfers.NoteTonnage.First(c =>
                            c.CategoryId.ToInt() == WeeeCategory.AutomaticDispensers.ToInt()).Id, 3, null)
                };
                ...
                var transferTonnages6 = new List<NoteTransferTonnage>()
                {
                    new NoteTransferTonnage(
                        evidenceNoteWithTransfers.NoteTonnage.First(c =>
                            c.CategoryId.ToInt() == WeeeCategory.AutomaticDispensers.ToInt()).Id, 4, null)
                };
                ...
                request = new GetEvidenceNotesSelectedForTransferRequest(recipientOrganisation.Id,
                    new List<Guid>(notesSetToBeIncluded.Select(n => n.Id)), categories, transferNote.Id);
            };
            ...
            private readonly It shouldHaveReturnedCorrectEvidenceNotes = () =>
            {
                ...
                note2.TotalReceivedAvailable.Should().Be(8); // total should include the note id passed to the request
        ...
        public class WhenIGetNotesSelectedToTransferForAnOrganisation : GetEvidenceNotesSelectedForTransferRequestHandlerIntegrationTestBase
        {
            ...
            private readonly It shouldHaveReturnedCorrectEvidenceNotes = () =>
            {
                ...
                var note2 = result.Results.First(r => r.Id.Equals(notesSetToBeIncluded.ElementAt(1).Id));
                note2.EvidenceTonnageData.Count.Should().Be(2);
                note2.TotalReceivedAvailable.Should().Be(5);

namespace EA.Weee.RequestHandlers.Tests.Unit.AatfReturn.SearchAatfAddress
{
    ...
    public class GetSearchAnAatfAddressHandlerTests
    {
        ...
        public async Task HandleAsync_GivenGetSearchAatfAddressRequest_DataAccessIsCalled()
        {
            ...
            A.CallTo(() => getSearchAnAatfAddressDataAccess.GetSearchAnAatfAddressBySearchTerm(getSearchAatfAddress)).MustHaveHappened(1, Times.Exactly);
        }

        [Fact]
        public async Task HandleAsync_GivenGetSearchAatfAddress_ReturnedListShouldContainElements()
        {
            var aatfId = Guid.NewGuid();
            var returnId = Guid.NewGuid();
        ...
        [Fact]
        public async Task HandleAsync_GivenGetSearchAatfAddress_ReturnedListShouldContainsOnly1Element()
        {
            var aatfId = Guid.NewGuid();
            var returnId = Guid.NewGuid();
            DateTime date = DateTime.Now;

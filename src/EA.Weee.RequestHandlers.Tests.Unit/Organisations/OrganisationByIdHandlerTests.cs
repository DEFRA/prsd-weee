namespace EA.Weee.RequestHandlers.Tests.Unit.Organisations
{
    ...
    public class OrganisationByIdHandlerTests
    {
        ...
        [Fact]
        public async Task HandleAsync_GivenPreviousYearSubmission_ShouldIncludeInMostRecentYear_RegardlessOfStatus()
        {
            // Arrange
            var organisation = GetOrganisationWithId(organisationId);
            var expectedReturnValue = new OrganisationData();
        ...
        [Fact]
        public async Task HandleAsync_GivenCurrentYearSubmission_ShouldOnlyIncludeIfComplete()
        {
            // Arrange
            var organisation = GetOrganisationWithId(organisationId);
            var expectedReturnValue = new OrganisationData();
            ...
            var submissions = new List<DirectProducerSubmission>
            {
                ...
                {
                    ComplianceYear = 2024,
                    DirectProducerSubmissionStatus = DirectProducerSubmissionStatus.Returned
                }
            };
        ...
        [Fact]
        public async Task HandleAsync_GivenMixedYearSubmissions_ShouldPreferCurrentYearCompleteOverPreviousYear()
        {
            // Arrange
            var organisation = GetOrganisationWithId(organisationId);
            var expectedReturnValue = new OrganisationData();
            ...
            // Assert
            result.DirectRegistrants.Single().MostRecentSubmittedYear.Should().Be(2024);
        }

        [Fact]
        public async Task HandleAsync_GivenMultiplePreviousYears_ShouldReturnMostRecent()
        {
            // Arrange
            var organisation = GetOrganisationWithId(organisationId);
            var expectedReturnValue = new OrganisationData();
        ...
        [Fact]
        public async Task HandleAsync_GivenFutureYearSubmission_ShouldNotIncludeUnlessComplete()
        {
            // Arrange
            var organisation = GetOrganisationWithId(organisationId);
            var expectedReturnValue = new OrganisationData();

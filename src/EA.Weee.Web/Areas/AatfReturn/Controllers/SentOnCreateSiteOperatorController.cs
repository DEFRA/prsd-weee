        ...
        public virtual async Task<ActionResult> Index(SentOnCreateSiteOperatorViewModel viewModel, FormCollection formCollection)
        {
            ...
            if (ModelState.IsValid)
            {
                using (var client = apiClient())
                {
                    ...
                    return AatfRedirect.ObligatedSentOn(viewModel.SiteAddressData.Name, viewModel.OrganisationId, viewModel.AatfId, viewModel.ReturnId, viewModel.WeeeSentOnId);
                }
            }

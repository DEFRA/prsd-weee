namespace EA.Weee.XmlValidation.BusinessValidation.MemberRegistration.Rules.Producer
{
    ...
    public class ProducerObligationTypeChange : IProducerObligationTypeChange
    {
        ...
        public async Task<RuleResult> Evaluate(producerType producer)
        {
            ...
            {
                var existingProducerDetails = producerQuerySet.GetLatestProducerForComplianceYearAndScheme(producer.registrationNo, complianceYear, organisationId);

                if (existingProducerDetails != null)
                {
                    result = await GetRuleResult(producer, existingProducerDetails);
                }
                else
                {
                    existingProducerDetails = producerQuerySet.GetLatestProducerFromPreviousComplianceYears(producer.registrationNo);
        ...
        private async Task<RuleResult> GetRuleResult(producerType producer, ProducerSubmission existingProducerDetails)
        {
            var result = RuleResult.Pass();
